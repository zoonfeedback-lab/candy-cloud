        ...
        <div className="max-w-7xl mx-auto space-y-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex flex-col justify-between relative overflow-hidden">
                    <div className="absolute top-4 right-4 text-pink-100/50 text-5xl font-black">💵</div>
                                    ...
                                    <tr key={order.id} className={i !== recentOrders.length - 1 ? "border-b border-gray-50" : ""}>
                                        ...
                                        <td className="py-4">
                                            <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold ${order.status === "delivered" ? "bg-gray-100 text-gray-500" :
                                                    order.status === "placed" ? "bg-pink-100 text-pink-600" :
                ...
                <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100 flex flex-col">
                    ...
                    <div className="mb-8">
                        ...
                        <div className="w-full bg-gray-100 rounded-full h-2">
                            <div className="bg-pink-500 h-2 rounded-full" style={{ width: "2%" }}></div>
                        </div>
                    </div>

                    <div className="bg-pink-50 border border-pink-100 rounded-2xl p-6 mb-8 mt-auto text-center">
                        <p className="text-xs font-black text-pink-400 uppercase tracking-widest mb-1">Current Jackpot</p>
                        ...
                        <p className="text-xs text-pink-600/80 font-medium">Remaining Scoops: {goldenScoopPanel?.scoopsRemaining ?? 0} / {goldenScoopPanel?.totalScoops ?? 60}</p>
                    </div>
                            ...
                            {goldenScoopPanel?.latestWinners && goldenScoopPanel.latestWinners.length > 0 ? (
                                <>
                                    {goldenScoopPanel.latestWinners.map((winner, idx) => {
                                        const bgColors = ["bg-pink-100 text-pink-600", "bg-teal-100 text-teal-600", "bg-pink-100 text-pink-600"];
                                        return (
                                    ...
                                    {kpis.goldenScoopsFound > 3 && (
                                        <div className="w-10 h-10 rounded-full bg-pink-100 border-2 border-white flex items-center justify-center text-xs font-black text-pink-600">
                                            +{kpis.goldenScoopsFound - 3}
                                        </div>
                                    )}
                                </>
                    ...
                    <button className="w-full bg-pink-500 hover:bg-pink-600 text-white font-bold py-3.5 rounded-xl transition-colors flex items-center justify-center gap-2 mt-auto">
            ...
            <div>
                <div className="flex justify-between items-end mb-6">
                    ...
                    <div className="flex gap-3">
                        ...
                        <button className="px-5 py-2.5 rounded-xl bg-pink-500 text-white font-bold text-sm hover:bg-pink-600 transition-colors">
                            Add Product
                        </button>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    {inventory && inventory.length > 0 ? inventory.slice(0, 3).map((item, i) => {
                        const colors = ["border-pink-500", "border-teal-400", "border-pink-500"];
                        const bgColors = ["bg-pink-500", "bg-teal-400", "bg-pink-500"];
                        const textColors = ["text-pink-500", "text-teal-400", "text-pink-500"];
